...
def test_v26135_metadata_reports_all_pathways_complete() -> None:
    metadata = load_metadata("26135")
    assert metadata.release_date == date(2026, 7, 9)
    assert "complete source-verified CO, SO, RE, ME, EV, and OU" in metadata.schema_completeness
    assert "event-output submode" in metadata.schema_completeness
